...
//    İşleme başlarken bir önceki değer olmadığı için bu değer identity parametresinde tanımlanmaktadır.
    ...
    public static void elemanlariTopla(List<Integer> sayilar) {
        ...
        System.out.println("sayilar.stream().reduce(Integer::sum) = " +
                sayilar.stream().reduce(Integer::sum).get()); //spesifik methoda daha hizlidir.
    }
    ...
    public static void elemanlariCarp(List<Integer> sayilar) {
        ...
        System.out.println("sayilar.stream().reduce(1,(a,b)-> a*b) = " +
                sayilar.stream().filter(Methodlarim::ciftElemaniBul).reduce(1, (a, b) -> a * b));

        //method ref kullanildi kendi class imizda
        //method referans ile

        System.out.println(sayilar.stream().filter(Methodlarim::ciftElemaniBul).reduce(Math::multiplyExact));

    }

    //TODO task teklerin karesini buyukten kucuge
    ...
    public static void strKarakterSayisiYazdir(List<String> yemek) {
        ...
        yemek.stream().sorted(Comparator.comparing(String::length).reversed()).
